...
export default function JapandiFeatures({ features, t }: FeaturesProps) {
  ...
  return (
    <section className="bg-kf-surface dark:bg-kf-foreground py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {list.map((f, i) => (
            <div key={i} className="text-center">
              <div className="w-14 h-14 mx-auto mb-6 rounded-full border border-kf-border flex items-center justify-center text-kf-primary text-xl">
                {f.icon}
              </div>
              <h3 className="text-base font-light tracking-[0.15em] uppercase text-kf-foreground dark:text-kf-bg mb-3" style={{ fontFamily: 'Georgia, serif' }}>{f.title}</h3>
