...
import { shareDriverOrder, type DriverOrder } from '../lib/formatDriverMessage.js';
...
export default function DriverWhatsAppModal({ order, onClose }: Props) {
  ...
  async function handleSend() {
    if (!order) return;
    setSending(true);
    // Mobile: share sheet do sistema (escolhe o app e depois o contato do entregador).
    // Desktop: abre o WhatsApp Web pedindo o contato.
    // Sempre SEM número fixo — nunca envia direto para o número do King.
    await shareDriverOrder(order);
    setSending(false);
    onClose();
  }
      ...
      >
        ...
        <div className="space-y-2">
          <button
            ...
            aria-label="Compartilhar comanda do pedido"
          >
            ...
            {sending ? "Preparando comanda..." : "Compartilhar comanda"}
          </button>
          <button
            onClick={onClose}
