            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {topLevel.map((cat) => (
                <tr key={cat.id} className="hover:bg-gray-50">
                  ...
                  <td data-label="Itens" className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {cat._count.menuItems}
                  </td>
                  <td data-label="Subcategories" className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  ...
                  <td data-label="Status" className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${cat.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                      ...
                      {cat.isActive ? 'Ativo' : 'Inactive'}
                    </span>
                  </td>
                  <td data-label="Order" className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {cat.sortOrder}
                  </td>
                  <td data-label="Ações" className="px-6 py-4 whitespace-nowrap text-right text-sm space-x-3">
                    <Link to={`/menu/categories?includeInactive=true/${cat.id}`} className="text-primary-600 hover:text-primary-900 font-medium" aria-label={`Edit category ${cat.name}`}>Editar</Link>
                    <button onClick={() => handleDelete(cat.id, cat.name)} className="text-red-600 hover:text-red-900 font-medium" aria-label={`Delete category ${cat.name}`}>Excluir</button>
                  </td>
                </tr>
              ))}
            </tbody>
